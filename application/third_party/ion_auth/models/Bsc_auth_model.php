...
class Bsc_auth_model extends CI_Model
{
	...
	public function __construct()
	{
		...
		$this->tables = $this->config->item('tables', 'bsc_auth');

		$this->identity_column = $this->config->item('identity', 'bsc_auth');
		$this->join = $this->config->item('join', 'bsc_auth');

		$this->hash_method = $this->config->item('hash_method', 'bsc_auth');

		$this->messages    = [];
		$this->errors      = [];
		$delimiters_source = $this->config->item('delimiters_source', 'bsc_auth');
		...
		{
			$this->message_start_delimiter = $this->config->item('message_start_delimiter', 'bsc_auth');
			$this->message_end_delimiter = $this->config->item('message_end_delimiter', 'bsc_auth');
			$this->error_start_delimiter = $this->config->item('error_start_delimiter', 'bsc_auth');
			$this->error_end_delimiter = $this->config->item('error_end_delimiter', 'bsc_auth');
		}
	...
	public function register($identity, $password, $email, $additional_data = [], $groups = [])
	{
		...
		$manual_activation = $this->config->item('manual_activation', 'bsc_auth');
		...
		else if (!$this->config->item('default_group', 'bsc_auth') && empty($groups))
		{
		...
		$query = $this->db->get_where($this->tables['groups'], ['name' => $this->config->item('default_group', 'bsc_auth')], 1)->row();
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($this->verify_password($password, $user->password, $identity))
			{
				...
				if ($this->config->item('remember_users', 'bsc_auth'))
				{
					if ($remember)
					{
	...
	public function recheck_session()
	{
		...
		$recheck = (NULL !== $this->config->item('recheck_timer', 'bsc_auth')) ? $this->config->item('recheck_timer', 'bsc_auth') : 0;
	...
	{
		if ($this->config->item('track_login_attempts', 'bsc_auth'))
		{
			$max_attempts = $this->config->item('maximum_login_attempts', 'bsc_auth');
			if ($max_attempts > 0)
			{
	...
	public function get_attempts_num($identity, $ip_address = NULL)
	{
		if ($this->config->item('track_login_attempts', 'bsc_auth'))
		{
			...
			if ($this->config->item('track_login_ip_address', 'bsc_auth'))
			{
				if (!isset($ip_address))
				{
					$ip_address = $this->input->ip_address();
			...
			$this->db->where('time >', time() - $this->config->item('lockout_time', 'bsc_auth'), FALSE);
	...
	{
		if ($this->config->item('track_login_attempts', 'bsc_auth') && $this->config->item('track_login_ip_address', 'bsc_auth'))
		{
	...
	public function clear_login_attempts($identity, $old_attempts_expire_period = 86400, $ip_address = NULL)
	{
		if ($this->config->item('track_login_attempts', 'bsc_auth'))
		{
			...
			$old_attempts_expire_period = max($old_attempts_expire_period, $this->config->item('lockout_time', 'bsc_auth'));
	...
	public function set_lang($lang = 'en')
	{
		...
		if($this->config->item('user_expire', 'bsc_auth') === 0)
		{
		...
		{
			$expire = $this->config->item('user_expire', 'bsc_auth');
		}
	...
	public function remember_user($identity)
	{
		...
		if ($token->validator_hashed)
		{
			...
			if ($this->db->affected_rows() > -1)
			{
				...
				set_cookie([
					'name'   => $this->config->item('remember_cookie_name', 'bsc_auth'),
	...
	public function login_remembered_user()
	{
		...
		$remember_cookie = get_cookie($this->config->item('remember_cookie_name', 'bsc_auth'));
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($this->verify_password($token->validator, $user->remember_code, $identity))
			{
				...
				if ($this->config->item('user_extend_on_login', 'bsc_auth'))
				{
					$this->remember_user($identity);
				}
	...
	public function update_group($group_id = FALSE, $group_name = FALSE, $additional_data = [])
	{
		...
		if ($this->config->item('admin_group', 'bsc_auth') === $group->name && $group_name !== $group->name)
		{
	...
	public function delete_group($group_id = FALSE)
	{
		...
		if($group->name == $this->config->item('admin_group', 'bsc_auth'))
		{
	...
	protected function _get_hash_parameters($identity = NULL)
	{
		...
		if ($identity)
		{
			...
			if ($user_id && $this->in_group($this->config->item('admin_group', 'bsc_auth'), $user_id))
			{
				$is_admin = TRUE;
			}
		}
		...
		switch ($this->hash_method)
		{
			case 'bcrypt':
				$params = [
					'cost' => $is_admin ? $this->config->item('bcrypt_admin_cost', 'bsc_auth')
										: $this->config->item('bcrypt_default_cost', 'bsc_auth')
				];
				break;

			case 'argon2':
				$params = $is_admin ? $this->config->item('argon2_admin_params', 'bsc_auth')
									: $this->config->item('argon2_default_params', 'bsc_auth');
				break;
	...
	protected function _password_verify_sha1_legacy($identity, $password, $hashed_password_db)
	{
		...
		if ($this->config->item('store_salt', 'bsc_auth'))
		{
		...
		else
		{
			...
			$salt_length = $this->config->item('salt_length', 'bsc_auth');
