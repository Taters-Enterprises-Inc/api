...
if ( ! function_exists('json_url'))
{
    ...
    {
        $CI =& get_instance();
        return base_url() . $CI->config->item('json_path');
    }
}
// ------------------------------------------------------------------------
// EMBED HELPERS
    ...
    function css($file, $media='all')
    {
        ...
        if(!is_array($file)){
            if(file_exists(css_path().$file)){
                $version =  '?v='.base_convert(filemtime(css_path().$file), 10, 36);
                $css = '<link rel="stylesheet" type="text/css" href="' . css_url() . $file . $version .'" media="' . $media . '">'."\n";
            }
        }else{
            foreach ( $file as $name ) {
                if(file_exists(css_path().$name)){
                    $version =  '?v='.base_convert(filemtime(css_path().$name), 10, 36);
                    $css = '<link rel="stylesheet" type="text/css" href="' . css_url() . $name . $version .'" media="' . $media . '">'."\n";
                }
            }
        }
        return $css;
    ...
    {   
        ...
        if(!is_array($file)){
            if(file_exists(js_path().$file)){
                $version =  '?v='.base_convert(filemtime(js_path().$file), 10, 36);
                $element = '<script type="text/javascript" src="' . js_url() . $file . $version . '"';
            ...
            foreach ( $file as $name ) {
                if(file_exists(js_path().$name)){
                    $version =  '?v='.base_convert(filemtime(js_path().$name), 10, 36);
                    $elem = '<script type="text/javascript" src="' . js_url() . $name . $version . '"';
                    foreach ( $atts as $key => $val )
                        $elem .= ' ' . $key . '="' . $val . '"';
                    $elem .= '></script>'."\n";
                    $element .= $elem;
